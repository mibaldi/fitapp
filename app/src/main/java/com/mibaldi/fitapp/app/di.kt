...
import com.mibaldi.fitapp.appData.database.RoomTrainingDataSource
import com.mibaldi.fitapp.appData.database.TrainingDatabase
...
val datasourcesModule = module {
    ...
    single { TrainingDatabase.build(get()) }
    single<CoroutineDispatcher> { Dispatchers.Main }
    single {AuthAppDb(get(named("baseUrl")))}
    single(named("baseUrl")) { "https://us-central1-fitapp-9fccb.cloudfunctions.net/api/" }
    //factory<RemoteDataSource> { FitAppDbDataSource(get()) }
    factory<RemoteDataSource> { FitAppDbDataSource(get()) }
    factory<LocationDataSource>{PlayServicesLocationDataSource(get()) }
    factory<PermissionChecker> {AndroidPermissionChecker(get())}
    factory<AuthRemoteDataSource> {AuthDbDataSource(get())}
    factory<LocalTrainingDataSource> { RoomTrainingDataSource(get()) }
}

val dataModule = module {
    factory { RegionRepository(get(),get()) }
    factory { TrainingsRepository(get(),get(),get()) }
...
private val scopesModule = module {
    ...
    scope(named<AdminActivity>()) {
        viewModel { com.mibaldi.fitapp.ui.admin.AdminViewModel(get(),get(),get()) }
        scoped { GetUsers(get()) }
        scoped { RemoveUserTrainings(get()) }
    }
}
